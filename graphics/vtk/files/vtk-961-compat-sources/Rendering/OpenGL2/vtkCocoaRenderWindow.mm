  ...
  if ((this->Size[0] != width) || (this->Size[1] != height) || this->GetParentId())
  {
      ...
      this->GetWindowCreated())
    {
      if (!resizing)
      {
        ...
        // convertRectFromBacking is available on NSWindow, use helper for NSView fallback
        NSRect newRect;
        if ([window respondsToSelector:@selector(convertRectFromBacking:)])
        {
          newRect = [window convertRectFromBacking:backingNewRect];
        }
        else
        {
          newRect = backingNewRect;
        }
  ...
  if ((this->Position[0] != x) || (this->Position[1] != y) || this->GetParentId())
  {
    ...
    else if (this->Mapped && this->GetRootWindow() && this->GetWindowCreated())
    {
      if (!resizing)
      {
        ...
        if ([window respondsToSelector:@selector(convertRectFromBacking:)])
        {
          newPosition = [window convertRectFromBacking:backingNewPosition];
        }
        else
        {
          newPosition = backingNewPosition;
        }
...
void vtkCocoaRenderWindow::CreateAWindow()
{
    ...
    this->GetConnectContextToNSView())
  {
    ...
    else
    {
      ...
      NSRect contentRect;
      if (screen && [screen respondsToSelector:@selector(convertRectFromBacking:)])
      {
  ...
  if (!this->GetWindowId() && this->GetConnectContextToNSView())
  {
    ...
    if (this->GetParentId())
    {
      ...
      if ([window respondsToSelector:@selector(convertRectFromBacking:)])
      {
        viewRect = [window convertRectFromBacking:backingViewRect];
      }
      else
      {
        viewRect = backingViewRect;
      }
...
int* vtkCocoaRenderWindow::GetPosition()
{
  ...
  else
  {
    ...
    if (window)
    {
      ...
      // Convert from points to pixels.
      NSRect backingWindowRect;
      if ([window respondsToSelector:@selector(convertRectToBacking:)])
      {
        backingWindowRect = [window convertRectToBacking:windowRect];
      }
      else
      {
        backingWindowRect = windowRect;
      }
